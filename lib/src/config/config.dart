...
class AppConfigLoader {
  static Future<AppConfig> load(
    ...
    String? _readString(YamlMap m, String key) =>
        m.containsKey(key) ? (m[key] as dynamic)?.toString() : null;
    bool? _readBool(YamlMap m, String key) =>
        m.containsKey(key) ? (m[key] as dynamic) == true : null;
    ...
    final transparent =
        cliOverrides?.transparent ?? (_readBool(map, 'transparent') ?? false);
    ...
    final format = (cliOverrides?.format ?? _readString(map, 'format') ?? 'png')
        .toLowerCase();
    ...
    final concurrency =
        cliOverrides?.concurrency ?? _readInt(map, 'concurrency') ?? 2;
    final negativePrompt = _readString(map, 'negative_prompt');

    final model =
        cliOverrides?.model ??
        _readString(map, 'model') ??
        (throw StateError('Missing required field: model'));
    final imagesRaw = map['images'];
    if (imagesRaw is! YamlMap || imagesRaw.isEmpty) {
      throw StateError('Missing or empty images map');
    }
    final images = <String, String>{
      for (final e in imagesRaw.entries) '${e.key}': '${e.value}',
    };

    final provider = model.split(':').firstOrNull ?? model;
    final keys = <String, String>{
      'openai':
          _readString(map, 'openai_api_key') ?? env['OPENAI_API_KEY'] ?? '',
      'gemini':
          _readString(map, 'gemini_api_key') ?? env['GEMINI_API_KEY'] ?? '',
    };
    ...
    final openaiProject =
        _readString(map, 'openai_project') ?? env['OPENAI_PROJECT'];
    final openaiOrganization =
        _readString(map, 'openai_organization') ??
