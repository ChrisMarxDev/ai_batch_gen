...
class GenerateCommand extends Command<int> {
  GenerateCommand({required Logger logger}) : _logger = logger {
    argParser
      ..addOption(
        'config',
        help: 'Path to YAML config (default ai_batch_gen.yaml)',
      )
      ..addOption('output', help: 'Output directory override')
      ..addOption(
        'model',
        help: 'Model override in the form <provider>:<model>',
      )
      ..addFlag(
        'transparent',
        help: 'Request transparent background (best-effort)',
      )
      ...
      ..addFlag(
        'ignore-failures',
        help: 'Exit 0 even if some images fail',
        negatable: false,
      );
  }
  ...
  Future<int> run() async {
    final configPath =
        (argResults?['config'] as String?) ?? 'ai_batch_gen.yaml';
    final loadProgress = _logger.progress(
      'Loading config ${cyan.wrap(configPath)}',
    );
    ...
    try {
      config = await AppConfigLoader.load(
        ...
        cliOverrides: CliOverrides(
          ...
          onlyKeys: (argResults?['only'] as String?)
              ?.split(',')
              .map((e) => e.trim())
  ...
  void _printPlan(AppConfig config) {
    ...
    _logger
      ...
      ..info(
        'Format: ${cyan.wrap(config.format)}  Size: ${cyan.wrap(config.size ?? 'default')}  Transparent: ${cyan.wrap(config.transparent.toString())}',
      )
      ..info(
        'Concurrency: ${cyan.wrap('${config.concurrency}')}  Timeout: ${cyan.wrap('${config.timeoutMs ?? 'default'} ms')}  Count: ${cyan.wrap('${config.count}')}',
      )
      ..info('Images: ${cyan.wrap('${config.images.length} entries')}');
    for (final entry in config.images.entries) {
      if (config.cli.onlyKeys != null &&
          !config.cli.onlyKeys!.contains(entry.key)) {
        continue;
      }
